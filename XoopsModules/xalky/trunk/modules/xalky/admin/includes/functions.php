...
function updateAdminLogin($data)
{
	...
	try {
		$dbh = db_connect();
		$params = array('');
		$query = "SELECT admin, adminLogin FROM xalky_config";							
...
function getAdminConfig()
{
	try {
		$dbh = db_connect();
		$params = array('');
		$query = "SELECT * FROM xalky_config";							
		$action = $dbh->prepare($query);
		$action->execute($params);
...
function updateAdminConfig($data)
{
	...
	// replace commas with ASCII characters
	$data['textAdvertsDesc'] = str_replace(",","&#44;",$data['textAdvertsDesc']);
	...
	try {
		...
		/* 
		* start badwords cleanup
		* ( remove any additional comma(s) left at the end/between badwords ) 
		*/
		$badwords_array = explode(",",$data['badwords']);
		$total_badwords = count($badwords_array);
		for($i = 0; $i < $total_badwords; ) {
		
			if( empty( $badwords_array[$i] ) ) {
				unset( $badwords_array[$i] );
			}
			
			$i++;
		}
		$data['badwords'] = implode(",",$badwords_array);
		/* end badwords cleanup */		
...
function updateAdminAdverts($data)
{
	if (isset($data['del']))
	{
		foreach ($data['del'] as $id)
		{
			try {
				...
				$query = "DELETE FROM xalky_adverts WHERE id = :id";							
				$action = $dbh->prepare($query);
				$action->execute($params);	
				$dbh = null;
			}
	...
	if (!empty($data['text']))
	{
		try {
			...
			$query = "INSERT INTO xalky_adverts
								(
									text, 
									displays,
									clicks
	...
	if (isset($data['del']))
	{
		foreach ($data['del'] as $id)
		{
			try {
				...
				$query = "DELETE FROM xalky_games WHERE game_ID = :id";							
				$action = $dbh->prepare($query);
				$action->execute($params);	
				$dbh = null;
			}
...
function getAdminRooms($id)
{
	...
	try {
		...
		if($id != '0')
		{
			...
			$query = "SELECT * FROM xalky_rooms WHERE id = :id";
		}
		else
		{
			$params = array('');
			$query = "SELECT * FROM xalky_rooms";		
		}
...
function getAdminGroups()
{
	...
	try {
		$dbh = db_connect();
		$params = array('');
		$query = "SELECT * FROM xalky_group ORDER BY id ASC";							
		$action = $dbh->prepare($query);
		$action->execute($params);
		...
		$html .= '<td>Allow Xalky</td>';
		$html .= '<td>Private Xalky</td>';
	...
	if (isset($data['del']))
	{
		foreach ($data['del'] as $id)
		{
			...
			try {
				...
				$query = "DELETE FROM xalky_group WHERE id = :id";							
				$action = $dbh->prepare($query);
				$action->execute($params);	
				$dbh = null;
			}
	...
	try {
		...
		foreach ($action as $i)
		{
			...
			else
			{
				...
				$html .= '<td>'.$i['email'].'</td>';
				$html .= '<td>'.$i['userIP'].'</td>';
				$html .= '<td align="center">'.$i['admin'].'</td>';		
				$html .= '<td align="center">'.$i['moderator'].'</td>';
				$html .= '<td align="center">'.$i['speaker'].'</td>';
	...
	if (isset($data['del']))
	{
		foreach ($data['del'] as $id)
		{
			...
			try {
				...
				$query = "DELETE FROM xalky_users WHERE id = :id";							
				$action = $dbh->prepare($query);
				$action->execute($params);	
				$dbh = null;
			}
			...
			try {
				...
				$query = "DELETE FROM xalky_profiles WHERE id = :id";							
				$action = $dbh->prepare($query);
				$action->execute($params);	
				$dbh = null;
			}
...
function sendAdminGroupEmail($data)
{
	...
	try {
		...
		{
			// send email
			mail(urldecode($i['email']), $data['subject'], $data['message'], $headers);

			$html .= "Email sent to: ".urldecode($i['email'])."<br>";
...
function getAdminDatabase()
{
	...
	try {
		...
		$query = "SELECT id FROM xalky_users WHERE active > :lastActive";							
		$action = $dbh->prepare($query);
		$action->execute($params);
		$count = $action->rowCount();		
	...
	if(file_exists("../plugins/adverts/index.php"))
	{
		try {
			$dbh = db_connect();
			$params = array('');
			$query = "SELECT id FROM xalky_adverts";							
			$action = $dbh->prepare($query);
			$action->execute($params);
			$count = $action->rowCount();		
		...
		$html .= '<tr><td>MySQL Table:</td><td>xalky_adverts</td></tr>';
		$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
		$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_adverts[]" value="TRUNCATE"></td></tr>';
		$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_adverts[]" value="OPTIMIZE"></td></tr>';
		$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
	}
	...
	if(file_exists("../plugins/games/index.php"))
	{
		try {
			$dbh = db_connect();
			$params = array('');
			$query = "SELECT game_ID FROM xalky_games";							
			$action = $dbh->prepare($query);
			$action->execute($params);
			$count = $action->rowCount();		
		...
		$html .= '<tr><td>MySQL Table:</td><td>xalky_games</td></tr>';
		$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
		$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_games[]" value="TRUNCATE"></td></tr>';
		$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_games[]" value="OPTIMIZE"></td></tr>';	
		$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
	}
	...
	try {
		$dbh = db_connect();
		$params = array('');
		$query = "SELECT id FROM xalky_message";							
		$action = $dbh->prepare($query);
		$action->execute($params);
		$count = $action->rowCount();		
	...
	$html .= '<tr><td>MySQL Table:</td><td>xalky_message</td></tr>';
	$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
	$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_message[]" value="TRUNCATE"></td></tr>';
	$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_message[]" value="OPTIMIZE"></td></tr>';	
	$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
	...
	if(file_exists("../plugins/moderated_chat/index.php"))
	{
		try {
			$dbh = db_connect();
			$params = array('');
			$query = "SELECT id FROM xalky_moderated";							
			$action = $dbh->prepare($query);
			$action->execute($params);
			$count = $action->rowCount();		
		...
		$html .= '<tr><td>MySQL Table:</td><td>xalky_moderated</td></tr>';
		$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
		$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_moderated[]" value="TRUNCATE"></td></tr>';
		$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_moderated[]" value="OPTIMIZE"></td></tr>';	
		$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
	}
	...
	if(file_exists("../plugins/one2one/index.html"))
	{
		try {
			$dbh = db_connect();
			$params = array('');
			$query = "SELECT id FROM xalky_one2one";							
			$action = $dbh->prepare($query);
			$action->execute($params);
			$count = $action->rowCount();		
		...
		$html .= '<tr><td>MySQL Table:</td><td>xalky_one2one</td></tr>';
		$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
		$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_one2one[]" value="TRUNCATE"></td></tr>';
		$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_one2one[]" value="OPTIMIZE"></td></tr>';	
		$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
	}
	...
	try {
		$dbh = db_connect();
		$params = array('');
		$query = "SELECT id FROM xalky_rooms";							
		$action = $dbh->prepare($query);
		$action->execute($params);
		$count = $action->rowCount();		
	...
	$html .= '<tr><td>MySQL Table:</td><td>xalky_rooms</td></tr>';
	$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
	$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_rooms[]" value="TRUNCATE"></td></tr>';
	$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_rooms[]" value="OPTIMIZE"></td></tr>';	
	$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
	...
	if(file_exists("../plugins/share/index.php"))
	{
		try {
			$dbh = db_connect();
			$params = array('');
			$query = "SELECT id FROM xalky_share";							
			$action = $dbh->prepare($query);
			$action->execute($params);
			$count = $action->rowCount();		
		...
		$html .= '<tr><td>MySQL Table:</td><td>xalky_share</td></tr>';
		$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
		$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_share[]" value="TRUNCATE"></td></tr>';
		$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_share[]" value="OPTIMIZE"></td></tr>';	
		$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
	}
	...
	try {
		$dbh = db_connect();
		$params = array('');
		$query = "SELECT id FROM xalky_users";							
		$action = $dbh->prepare($query);
		$action->execute($params);
		$count = $action->rowCount();		
	...
	$html .= '<tr><td>MySQL Table:</td><td>xalky_users</td></tr>';
	$html .= '<tr><td>Total Entries:</td><td>'.$count.'</td></tr>';
	$html .= '<tr><td>Truncate:</td><td><input type="checkbox" name="xalky_users[]" value="TRUNCATE"></td></tr>';
	$html .= '<tr><td>Optimize:</td><td><input type="checkbox" name="xalky_users[]" value="OPTIMIZE"></td></tr>';	
	$html .= '<tr><td colspan="2">&nbsp;</td></tr>';
...
{
	if (!empty($data['xalky_adverts']))
	{
		foreach ($data['xalky_adverts'] as $id)
		{
			try {
				$dbh = db_connect();
				$params = array('');
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_adverts";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_adverts";					
				}				
	...
	if (!empty($data['xalky_games']))
	{
		foreach ($data['xalky_games'] as $id)
		{
			try {
				$dbh = db_connect();
				$params = array('');
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_games";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_games";					
				}				
	...
	if (!empty($data['xalky_message']))
	{
		foreach ($data['xalky_message'] as $id)
		{
			try {
				$dbh = db_connect();
				$params = array('');
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_message";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_message";					
				}				
	...
	if (!empty($data['xalky_moderated']))
	{
		foreach ($data['xalky_moderated'] as $id)
		{
			try {
				$dbh = db_connect();
				$params = array('');
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_moderated";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_moderated";					
				}				
	...
	if (!empty($data['xalky_one2one']))
	{
		foreach ($data['xalky_one2one'] as $id)
		{
			try {
				$dbh = db_connect();
				$params = array('');
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_one2one";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_one2one";					
				}				
	...
	if (!empty($data['xalky_rooms']))
	{
		foreach ($data['xalky_rooms'] as $id)
		{
			try {
				$dbh = db_connect();
				$params = array('');
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_rooms";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_rooms";					
				}				
	...
	if (!empty($data['xalky_share']))
	{
		foreach ($data['xalky_share'] as $id)
		{
			try {
				$dbh = db_connect();
				$params = array('');
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_share";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_share";					
				}				
	...
	if (!empty($data['xalky_users']))
	{
		foreach ($data['xalky_users'] as $id)
		{
			// users table
			try {
				$dbh = db_connect();
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_users";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_users";					
				}				
			...
			try {
				...
				if($id == "TRUNCATE")
				{
					$query = "TRUNCATE TABLE xalky_profiles";					
				}
				
				if($id == "OPTIMIZE")
				{
					$query = "OPTIMIZE TABLE xalky_profiles";					
				}				
...
{
	$html  = '';
	$html .= '<tr><td class="header" colspan="2">Add New User</td></tr>';
	$html .= '<tr><td width="100">Username</td><td><input type="text" name="uName" value="" maxlength="16" autocomplete="off"></td></tr>';
	$html .= '<tr><td>Password</td><td><input type="password" name="uPass" value="" autocomplete="off"></td></tr>';	
	$html .= '<tr><td>Email</td><td><input type="text" name="email" value="" autocomplete="off"> (optional)</td></tr>';		
	$html .= '<tr><td>&nbsp;</td><td><input style="cursor:pointer;" class="submit" type="submit" name="submit" value="Update"></td></tr>';
...
{	
	...
	include(dirname(dirname(dirname(__FILE__)))."/lang/english.php");	
	return registerUser($data['uName'],$data['uPass'],$data['email']);
}
...
function getSubs($data)
{
	...
	$html .= '<tr><td class="header" colspan="9">Active Subscriptions</td></tr>';
	$html .= '<tr>';
	$html .= '<td>ID</td>';		
	$html .= '<td>Username</td>';				
	$html .= '<td>User ID</td>';				
	...
	try {
		...
		{
			$params = array('');
			$query = "SELECT * FROM xalky_subs ORDER BY id DESC";	
		}
		$action = $dbh->prepare($query);
		$action->execute($params);
		...
		{
			$html .= '<tr>';	
				$html .= '<td style="width:10px;">'.$i['id'].'</td>';				
				$html .= '<td>'.$i['username'].'</td>';				
				$html .= '<td>'.$i['userid'].'</td>';				
				$html .= '<td>'.$i['subid'].'</td>';	
				$html .= '<td>'.$i['day_pass'].'</td>';		
				$html .= '<td>'.$i['subscription'].'</td>';				
				$html .= '<td>'.date("H:i d-m-Y", $i['sub_start']).'</td>';				
				$html .= '<td>'.date("H:i d-m-Y",$i['sub_expires'] + $i['sub_start']).'</td>';
				$html .= "<td><input onclick=\"confirmDelete('".$i['id']."');\" style=\"cursor:pointer;\" class=\"submit\" type=\"button\" name=\"button\" value=\"Delete\"></td>";				
			$html .= '</tr>';		
		}
...
function delSubs($data)
{
	try {
		...
		$query = "DELETE FROM xalky_subs WHERE id = :id";								
		$action = $dbh->prepare($query);
		$action->execute($params);			
		$dbh = null;
	}				
	...
	$html = "This entry has been deleted.";
