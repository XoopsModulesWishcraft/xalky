...
# XML Stream Handler class used to parse xalky messages:
class XMLStreamHandler
 attr_reader :type,:xalky_id,:user_id,:reg_id,:channel_id,:channel_ids
 ...
 @type = :message
 @xalky_id = attrs['xalkyID']
 @channel_id = attrs['channelID']
 throw :break
 when 'register'
 # register messages are sent by xalky clients:
 @type = :register
 @xalky_id = attrs['xalkyID']
 ...
 throw :break
 when 'authenticate'
 # authenticate messages are sent by the xalky server client:
 @type = :authenticate
 @xalky_id = attrs['xalkyID']
 @user_id = attrs['userID']
 @reg_id = attrs['regID']
...
class SocketServer
 ...
 # Clients list:
 @clients = Hash::new
 # Chats list, used to distinguish between different xalky installations (contains channels list):
 @xalkys = Hash::new
 ...
 @config[:eol] = "\0"
 # Log level (0 logs only errors and server start/stop, 1 logs client connections, 2 logs all messages but no broadcast xalky, 3 logs everything):
 @config[:log_level] = 0
 end
 ...
 case handler.type
 when :message
 handle_broadcast_message(client_socket, handler.xalky_id, handler.channel_id, str)
 when :register
 handle_client_registration(client_socket, handler.xalky_id, handler.user_id, handler.reg_id)
 when :authenticate
 handle_client_authentication(client_socket, handler.xalky_id, handler.user_id, handler.reg_id, handler.channel_ids)
 ...
 def handle_broadcast_message(client_socket, xalky_id, channel_id, str)
 # Check if the_client is allowed to broadcast:
 if @clients[client_socket][:allowed_to_broadcast]
 # Check if the xalky and channel have been registered:
 if @xalkys[xalky_id] && (@xalkys[xalky_id][channel_id] || @xalkys[xalky_id]['ALL'])
 ...
 if @xalkys[xalky_id]['ALL']
 reg_id = @xalkys[xalky_id]['ALL'][@clients[socket][:user_id]]
 end
 if !reg_id && @xalkys[xalky_id][channel_id]
 reg_id = @xalkys[xalky_id][channel_id][@clients[socket][:user_id]]
 end
 ...
 puts "#{Time.now}\t#{@clients[client_socket][:id]} Chat:#{xalky_id.to_s.dump} Channel:#{channel_id.to_s.dump} Message:#{str.to_s.dump}"; STDOUT.flush
 elsif @config[:log_level].to_i > 1
 # Log the message sent by the broadcast client:
 puts "#{Time.now}\t#{@clients[client_socket][:id]} Chat:#{xalky_id.to_s.dump} Channel:#{channel_id.to_s.dump} Message"; STDOUT.flush
 end
 end
 end
 
 def handle_client_registration(client_socket, xalky_id, user_id, reg_id)
 # Save the xalky_id, use_id and reg_id as client properties:
 @clients[client_socket][:xalky_id] = xalky_id
 ...
 puts "#{Time.now}\t#{@clients[client_socket][:id]} Chat:#{xalky_id.to_s.dump} User:#{user_id.to_s.dump} Reg:#{reg_id.to_s.dump}"; STDOUT.flush
 end
 end
 
 def handle_client_authentication(client_socket, xalky_id, user_id, reg_id, channel_ids)
 # Only the broadcast clients may send authentication messages:
 if @clients[client_socket][:allowed_to_broadcast]
 # Create a new xalky item if not found for the given xalky_id:
 if !@xalkys[xalky_id]
 @xalkys[xalky_id] = Hash.new
 end
 # Go through the list of channels for the given xalky:
 @xalkys[xalky_id].each_key do |key|
 # Delete all items for the given user on all channels of the given xalky:
 @xalkys[xalky_id][key].delete(user_id)
 # If the xalky channel is empty, delete the channel item:
 if @xalkys[xalky_id][key].size == 0
 @xalkys[xalky_id].delete(key)
 end
 end
 # Go through the list of authenticated channel_ids:
 channel_ids.each do |channel_id|
 # Create a new channel item if not found for the current channel_id (and the given xalky_id):
 if !@xalkys[xalky_id][channel_id]
 @xalkys[xalky_id][channel_id] = Hash.new
 end
 # Add a user item of the given user_id with the given reg_id to the current channel:
 @xalkys[xalky_id][channel_id][user_id] = reg_id
 end
 if @config[:log_level].to_i > 1
 # Log the client authentication:
 puts "#{Time.now}\t#{@clients[client_socket][:id]} Chat:#{xalky_id.to_s.dump} User:#{user_id.to_s.dump} Auth:#{reg_id.to_s.dump} Channels:#{channel_ids.join(',').dump}"; STDOUT.flush
 end
 end
 end
