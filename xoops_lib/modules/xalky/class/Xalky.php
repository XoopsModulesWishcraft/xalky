...
class Xalky {
	...
	function initConfig() {
		$config = null;
		if (!include(_XALKY_PATH . '/include/config.php')) {
			echo('<strong>Error:</strong> Could not find a config.php file in "'._XALKY_PATH.'"lib/". Check to make sure the file exists.');
			die();
		}
		$this->_config = $config;
	...
	function initRequestVars() {
		...
		$this->_requestVars['xalky']			= isset($_REQUEST['xalky'])			? true							: false;
	...
	function initSession() {
		...
		if($this->isLoggedIn()) {
			// Logout if we receive a logout request, the xalky has been closed or the userID could not be revalidated:
			...
			if($this->getConfig('ipCheck') && ($this->getSessionIP() === null || $this->getSessionIP() != $this->_cipaddy)) {
				$this->logout('IP');
				return;
			}
		} else if(
	...
	function updateLogsViewSocketAuthentication() {
		if($this->getUserRole() != _XALKY_ADMIN) {
			$channels = array();
	...
	function initChatViewSession() {
		// If channel is not null we are logged in to the xalky view:
		if($this->getChannel() !== null) {
			...
			if($this->getRequestVar('xalky')) {
				// Set channel, insert login messages and add to online list on first xalky request in xalky view:
				$this->xalkyViewLogin();
			}
		}
	}
	...
	function handleRequest() {
		if($this->getRequestVar('xalky')) {
			...
			// Send xalky messages and online user list in XML format:
			$this->sendXMLMessages();
		} else {
			// Display XHTML content for non-xalky requests:
	...
	function sendXHTMLContent() {
		$httpHeader = new XalkyHTTPHeader($this->getConfig('contentEncoding'), $this->getConfig('contentType'));
		$template = new XalkyTemplate($this, $this->getTemplateFileName(), $httpHeader->getContentType());
		$httpHeader->send();		
		$template->display();
	}
	...
	function getTemplateFileName() {
		switch($this->getView()) {
			case 'xalky':
				return _XALKY_TEMPLATE_PATH.'xalky_loggedin.html';
			default:
				return _XALKY_TEMPLATE_PATH.'xalky_loggedout.html';
		}
	}

	function initView() {
		$this->_view = null;
		// "xalky" is the default view:
		$view = ($this->getRequestVar('view') === null) ? 'xalky' : $this->getRequestVar('view');
	...
	function hasAccessTo($view) {
		switch($view) {
			...
			case 'teaser':
				...
				return false;
			case 'logs':
				if($this->isLoggedIn() && ($this->getUserRole() == _XALKY_ADMIN ||
					($this->getConfig('logsUserAccess') &&
					($this->getUserRole() == _XALKY_MODERATOR || $this->getUserRole() == _XALKY_USER))
					)) {
					return true;
				}
				return false;
	...
	function login() {
		...
		// If the xalky is closed, only the admin may login:
		if(!$this->isChatOpen() && $userData['userRole'] != _XALKY_ADMIN) {
			$this->addInfoMessage('errorChatClosed');
			return false;
		}
		
		if(!$this->getConfig('allowGuestLogins') && $userData['userRole'] == _XALKY_GUEST) {
			return false;
		}
		...
		if($this->isUserOnline($userData['userID']) || $this->isUserNameInUse($userData['userName'])) {
			if($userData['userRole'] == _XALKY_USER || $userData['userRole'] == _XALKY_MODERATOR || $userData['userRole'] == _XALKY_ADMIN) {
		...
		if($userData['userRole'] != _XALKY_ADMIN && $this->isUserBanned($userData['userName'], $userData['userID'], $this->_cipaddy)) {
			$this->addInfoMessage('errorBanned');
			return false;
		}
		
		// Check if the max number of users is logged in (not affecting moderators or admins):
		if(!($userData['userRole'] == _XALKY_MODERATOR || $userData['userRole'] == _XALKY_ADMIN) && $this->isMaxUsersLoggedIn()) {
			$this->addInfoMessage('errorMaxUsersLoggedIn');
			return false;
		}

		// Use a new session id (if session has been started by the xalky):
		...
		$this->setSessionIP($this->_cipaddy);
	...
	function addToOnlineList() {
		$onlineHandler = xoops_getmodulehandler('online', 'xalky');
		$online = $onlineHandler->create();
		$online->setVar('userID', $this->db->makeSafe($this->getUserID()));
		$online->setVar('userName', $this->db->makeSafe($this->getUserName()));
		$online->setVar('userRole', $this->db->makeSafe($this->getUserRole()));
		$online->setVar('channel', $this->db->makeSafe($this->getChannel()));
		$online->setVar('time', time());
		$online->setVar('ip', $this->db->makeSafe($this->ipToStorageFormat($this->_cipaddy)));
		if ($onlineHandler->insert($online, true))
			$this->resetOnlineUsersData();
	}
	
	function removeFromOnlineList() {
		$onlineHandler = xoops_getmodulehandler('online', 'xalky');
		$criteria = new Criteria('userID', $this->db->makeSafe($this->getUserID()));
		if ($onlineHandler->deleteAll($criteria))
	...
	function updateOnlineList() {
		$onlineHandler = xoops_getmodulehandler('online', 'xalky');
		$criteria = new Criteria('userID', $this->db->makeSafe($this->getUserID()));
		if ($onlines = $onlineHandler->getObject($criteria, false, false))
		{
			$onlines[0]->setVar('userName', $this->db->makeSafe($this->getUserName()));
			$onlines[0]->setVar('channel', $this->db->makeSafe($this->getChannel()));
			$onlines[0]->setVar('time', time());
			$onlines[0]->setVar('ip', $this->db->makeSafe($this->ipToStorageFormat($this->_cipaddy)));
			$onlineHandler->insert($onlines[0], true);
			$this->resetOnlineUsersData();
		}
	}
	
	function initMessageHandling() {
		// Don't handle messages if we are not in xalky view:
		if($this->getView() != 'xalky') {
			return;
		}
	...
	function insertParsedMessage($text) {
		...
		if(strpos($text, '/') === 0) {
			...
			switch($textParts[0]) {
				...
				// Kicking offending users from the xalky:
				case '/kick':
	...
	function insertParsedMessageKick($textParts) {
		...
		if($this->getUserRole() == _XALKY_ADMIN || $this->getUserRole() == _XALKY_MODERATOR) {
			if(count($textParts) == 1) {
				$this->insertChatBotMessage(
					$this->getPrivateMessageID(),
					'/error MissingUserName'
				...
				if($kickUserID === null) {
					...
					if($kickUserRole == _XALKY_ADMIN || ($kickUserRole == _XALKY_MODERATOR && $this->getUserRole() != _XALKY_ADMIN)) {
	...
	function insertParsedMessageNick($textParts) {
		if(!$this->getConfig('allowNickChange') ||
			(!$this->getConfig('allowGuestUserName') && $this->getUserRole() == _XALKY_GUEST)) {
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				'/error CommandNotAllowed '.$textParts[0]
			);
			...
			if($newUserName == $this->getLoginUserName()) {
				...
				$prefix = '';
				$suffix = '';
			} else if($this->getUserRole() == _XALKY_GUEST) {
	...
	function deleteMessage($messageID) {
		$messageHandler = xoops_getmodulehandler('messages', 'xalky');
		$criteria = new Criteria('id', $messageID);
		if ($messages = $messageHandler->getObject($criteria, false, false))
			if (isset($messages[0]))
				$row = $messages[0]->getValues();
		...
		if($row['channel'] !== null) {
			...
			if($this->getUserRole() == _XALKY_ADMIN) {
				$condition = '';
			} else if($this->getUserRole() == _XALKY_MODERATOR) {
				$criteria->add(new Criteria('userRole', _XALKY_ADMIN, '='), 'OR');
				$criteria->add(new Criteria('userRole', _XALKY_CHATBOT, '='), 'OR');
			} else if($this->getUserRole() == _XALKY_USER && $this->getConfig('allowUserMessageDelete')) {
				$criteria->add(new Criteria('userID', $this->getUserID()), 'NOT');
				$criteralpha = new CriteriaCompo(new Criteria('userRole', _XALKY_ADMIN, '!='), 'AND');
				$criteralpha->add(new Criteria('userRole', _XALKY_CHATBOT, '!='), 'AND');
				$criteromega = new CriteriaCompo(new Criteria('channel', $this->getPrivateMessageID(), '!='), 'OR');
				$criteromega->add(new Criteria('userRole', $this->getPrivateChannelID(), '!='), 'AND');
				$criteromega->add($criteralpha);
				$criteria->add($criteromega);
				
			} else {
				return false;
			}
	
			if($messageHandler->deleteall($criteria)) {
				// Insert a deletion command to remove the message from the clients xalkylists:
	...
	function floodControl() {
		...
		if($this->getUserRole() == _XALKY_MODERATOR || $this->getUserRole() == _XALKY_ADMIN) {
			return true;
		}
		$time = time();
	...
	function insertChatBotMessage($channelID, $messageText, $ip=null, $mode=0) {
		$this->insertCustomMessage(
			$this->getConfig('xalkyBotID'),
			$this->getConfig('xalkyBotName'),
			_XALKY_CHATBOT,
			$channelID,
			$messageText,
			$ip,
	...
	function insertCustomMessage($userID, $userName, $userRole, $channelID, $text, $ip=null, $mode=0) {
		...
		$ip = $ip ? $ip : $this->_cipaddy;
		
		$messageHandler = xoops_getmodulehandler('messages', 'xalky');
		$message = $messageHandler->create();
		$message->setVar('userID', $userID);
		$message->setVar('userName', $userName);
		$message->setVar('userRole', $userRole);
		$message->setVar('channel', $channelID);
		$message->setVar('time', time());
		$message->setVar('ip', $ip);
		$message->setVar('text', $text);
	...
	function getSocketBroadcastMessage(
		...
		$xml = '<root xalkyID="'.$this->getConfig('socketServerChatID').'" channelID="'.$channelID.'" mode="'.$mode.'">';
		if($mode) {
	...
	function updateSocketAuthentication($userID, $socketRegistrationID=null, $channels=null) {
		...
		$authentication = '<authenticate xalkyID="'.$this->getConfig('socketServerChatID').'" userID="'.$userID.'" regID="'.$socketRegistrationID.'">';
		if($channels) {
	...
	function getBannedUsersData($key=null, $value=null) {
		if($this->_bannedUsersData === null) {
			...
			$bansHandler = xoops_getmodulehandler('bans', 'xalky');
			$criteria = new Criteria('time', time(), '>=');
			if ($bans = $bansHandler->getObjects($criteria, true))
			{
				foreach($bans as $id => $ban)
				{
					$row = $ban->getValues();
					$row['ip'] = $this->ipFromStorageFormat($ban->getVar('ip'));
	...
	function banUser($userName, $banMinutes=null, $userID=null) {
		...
		$bansHandler = xoops_getmodulehandler('bans', 'xalky');
		$ban = $bansHandler->create();
		$ban->setVar('userID', $userID);
		$ban->setVar('userName', $userName);
		$ban->setVar('time', time());
		$ban->setVar('ip', $ip);
		$bansHandler->insert($ban);
	}
	
	function unbanUser($userName) {
		$bansHandler = xoops_getmodulehandler('bans', 'xalky');
		$criteria = new Criteria('userName', $userName, '=');
		$bansHandler->deleteAll($criteria);
	}
	...
	function setInactive($userID, $userName=null) {
		...
		$onlineHandler = xoops_getmodulehandler('online', 'xalky');
		$onlineHandler->setUserInactive($userID, $userName, $this->getConfig('inactiveTimeout'));
		$this->resetOnlineUsersData();
	}
	
	function removeInactive() {
		$onlineHandler = xoops_getmodulehandler('online', 'xalky');
		$criteria = new CriteriaCompo(new Criteria('time', time(), '<='));
		
		// Create a new SQL query:
		$onliners = $onlineHandler->getObjects($criteria, false);
		...
		if(count($onliners) > 0) {
			$condition = new CriteriaCompo();
			foreach($onliners as $key => $online) {
				$row = $online->getValues();
				$condition->add(new Criteria('userID',$row['userID']), 'OR');
			...
			$onlineHandler->deleteAll($condition);
		}
	}

	function updateOnlineStatus() {
		// Update online status every 50 seconds (this allows update requests to be in time):
		if(!$this->getStatusUptimeStamp() || ((time() - $this->getStatusUptimeStamp()) > 50)) {
			$this->updateOnlineList();
			$this->setStatusUptimeStamp(time());
		}
	}
	...
	function sendXMLMessages() {		
		$httpHeader = new XalkyHTTPHeader('UTF-8', 'text/xml');
	...
	function getMessageCondition() {
		$condition = new CriteriaCompo(new Criteria('id', $this->getRequestVar('lastID'), '>'), 'AND');
		$omega = new CriteriaCompo(new Criteria('channel', $this->getChannel()), 'OR');
		$omega->add(new Criteria('channel', $this->getPrivateMessageID()), 'OR');
		$condition->add($omega);
		if($this->getConfig('requestMessagesPriorChannelEnter') ||
			...
			$condition->add(new Criteria('time', time() + ($this->getConfig('requestMessagesTimeDiff') * 3600), '<='), 'AND');
		} else {
			$condition->add(new Criteria('time', $this->getChannelEnterTimeStamp(), '>='), 'AND');
		}
		return $condition;
	}
	
	function getMessageFilter() {
		$filterChannelMessages  = new CriteriaCompo(null, 'AND NOT');
		if(!$this->getConfig('showChannelMessages') || $this->getRequestVar('shoutbox')) {
			$filterChannelMessages->add(new Criteria('text', "%login%", 'LIKE'), 'OR');
			$filterChannelMessages->add(new Criteria('text', "%logout%", 'LIKE'), 'OR');
			$filterChannelMessages->add(new Criteria('text', "%channelEnter%", 'LIKE'), 'OR');
			$filterChannelMessages->add(new Criteria('text', "%channelLeave%", 'LIKE'), 'OR');
			$filterChannelMessages->add(new Criteria('text', "%kick%", 'LIKE'), 'OR');
		}
	...
	function getChatViewMessagesXML() {
		...
		$messagesHandler = xoops_getmodulehandler('messages', 'xalky');
		$criteria = new CriteriaCompo($this->getMessageCondition());
		$criteria->add($this->getMessageFilter());
		$criteria->setLimit($this->getConfig('requestMessagesLimit'));
		if ($objects = $messagesHandler->getObjects($criteria, true)) {
			$messages = '';
			// Add the messages in reverse order so it is ascending again:
			foreach($objects as $key => $object) {			
				$message = $this->getChatViewMessageXML(
					$object->getVar('id'),
					$object->getVar('timeStamp'),
					$object->getVar('userID'),
					$object->getVar('userName'),
					$object->getVar('userRole'),
					$object->getVar('channelID'),
					$object->getVar('text')
				);		
	...
	function getTeaserMessageCondition() {
		$condition = new CriteriaCompo(new Criteria('channel', $this->getValidRequestChannelID(), '>'), 'AND');
		if($this->getConfig('requestMessagesPriorChannelEnter') ||
		($this->getConfig('requestMessagesPriorChannelEnterList') && in_array($this->getChannel(), $this->getConfig('requestMessagesPriorChannelEnterList')))) {
			$condition->add(new Criteria('time', time() + ($this->getConfig('requestMessagesTimeDiff') * 3600), '<='), 'AND');
		} else {
			$condition->add(new Criteria('0', '1'), 'AND');
		}
		return $condition;
	}
	...
	function getTeaserViewMessagesXML() {
		$messagesHandler = xoops_getmodulehandler('messages', 'xalky');
		$criteria = new CriteriaCompo($this->getTeaserMessageCondition());
		$criteria->setLimit($this->getConfig('requestMessagesLimit'));
		if ($objects = $messagesHandler->getObjects($criteria, true)) {
			$messages = '';
			// Add the messages in reverse order so it is ascending again:
			foreach($objects as $key => $object) {			
				...
				$message = '';
				$message .= '<message';
				$message .= ' id="'.$object->getVar('id').'"';
				$message .= ' time="'.date('r', $object->getVar('timeStamp')).'"';
				$message .= ' userID="'.$object->getVar('userID').'"';
				$message .= ' userRole="'.$object->getVar('userRole').'"';
				$message .= ' channelID="'.$object->getVar('channelID').'"';
				$message .= '>';
				$message .= '<username><![CDATA['.$this->encodeSpecialChars($object->getVar('userName')).']]></username>';
				$message .= '<text><![CDATA['.$this->encodeSpecialChars($object->getVar('text')).']]></text>';
				$message .= '</message>';
				$messages = $message.$messages;
			}
		}
	...
	function getOnlineUsersData($channelIDs=null, $key=null, $value=null) {
		if($this->_onlineUsersData === null) {
			...
			$onlineHandler = xoops_getmodulehandler('online', 'xalky');
			$criteria = new Criteria('userName', '%', "LIKE");
			$criteria->setSort('`userName`');
			$onliners = $onlineHandler->getObjects($criteria, true);
			foreach( $onliners as $key => $onliner) 
			{
				$row = $onliner->getValues();
	...
	function getInvitations() {
		if($this->_invitations === null) {
			...
			$invitationsHandler = xoops_getmodulehandler('invitations', 'xalky');
			$criteria = new CriteriaCompo(new Criteria('userID', $this->getUserID()));
			$criteria->add(new Criteria('time', time()- 3600*24, '>='));
			$invitations = $invitationsHandler->getObjects($criteria, false);
			foreach($invitations as $invitation) {
				array_push($this->_invitations, $invitation->getVar('channel'));
			}
		}
	...
	function removeExpiredInvitations() {
		$invitationsHandler = xoops_getmodulehandler('invitations', 'xalky');
		$criteria = new Criteria('time', time()- 3600*24, '<=');
		$invitationsHandler->deleteAll($criteria);
	}
	...
	function addInvitation($userID, $channelID=null) {
		...
		$invitationsHandler = xoops_getmodulehandler('invitations', 'xalky');
		$invitation = $invitationsHandler->create();
		$invitation->setVar('userID', $userID);
		$invitation->setVar('channel', $channelID);
		$invitation->setVar('time', time());
		$invitationsHandler->insert($invitation);
	...
	function removeInvitation($userID, $channelID=null) {
		$channelID = ($channelID === null) ? $this->getChannel() : $channelID;
		$invitationsHandler = xoops_getmodulehandler('invitations', 'xalky');
		$criteria = new CriteriaCompo(new Criteria('userID', $userID));
		$criteria->add(new Criteria('channel', $channelID));
		$invitationsHandler->deleteAll($criteria);
	}
	...
	function getStatusUptimeStamp() {
		return $this->getSessionVar('StatusUptimeStamp');
	}

	function setStatusUptimeStamp($time) {
		$this->setSessionVar('StatusUptimeStamp', $time);
	}
	...
	function getLangCode() {
		...
		if(!in_array($langCode, $this->getConfig('langAvailable'))) {
			...
			$language = new XalkyLanguage($this->getConfig('langAvailable'), $this->getConfig('langDefault'));
	...
	function removeUnsafeCharacters($str) {
		...
		return XalkyEncoding::removeUnsafeCharacters($str);
	}

	function subString($str, $start=0, $length=null, $encoding='UTF-8') {
		return XalkyString::subString($str, $start, $length, $encoding);
	}
	
	function stringLength($str, $encoding='UTF-8') {
		return XalkyString::stringLength($str, $encoding);
	}
	...
	function convertEncoding($str, $charsetFrom, $charsetTo) {
		return XalkyEncoding::convertEncoding($str, $charsetFrom, $charsetTo);
	}

	function encodeEntities($str, $encoding='UTF-8', $convmap=null) {
		return XalkyEncoding::encodeEntities($str, $encoding, $convmap);
	}

	function decodeEntities($str, $encoding='UTF-8', $htmlEntitiesMap=null) {
		return XalkyEncoding::decodeEntities($str, $encoding, $htmlEntitiesMap);
	}
	
	function htmlEncode($str) {
		return XalkyEncoding::htmlEncode($str, $this->getConfig('contentEncoding'));
	}
	...
	function decodeSpecialChars($str) {
		return XalkyEncoding::decodeSpecialChars($str);
	}
	...
	function getLang($key=null) {
		if(!$this->_lang) {
			...
			$lang = null;
			require(_XALKY_LIB_PATH . 'lang/'.$this->getLangCode().'.php');
			$this->_lang = $lang;
		}
		if($key === null)
			return $this->_lang;
	...
	function isAllowedToSendPrivateMessage() {
		if($this->getConfig('allowPrivateMessages') || $this->getUserRole() == _XALKY_ADMIN) {
			return true;
		}
		return false;
	}
	...
	function getGuestUser() {
		...
		$userData['userRole'] = _XALKY_GUEST;
		return $userData;		
	}
	...
	// or the user is authenticated as guest in the xalky and the authentication system
	...
	function getValidLoginUserData() {
		...
		if(false) {
			...
			{
				$user = array('userID'=>$GLOBALS['xoopsUser']->getVar('uid'), 'userName'=>$GLOBALS['xoopsUser']->getVar('uid'));
				$groups = $GLOBALS['xoopsUser']->getGroups();
				if (!isset($user['userRole']))
					foreach($GLOBALS['xoopsModuleConfig']['admingroups'] as $groupid)
						if (in_array($groupid, $groups))
							$user['userRole'] = _XALKY_ADMIN;
				if (!isset($user['userRole']))
					foreach($GLOBALS['xoopsModuleConfig']['modgroups'] as $groupid)
						if (in_array($groupid, $groups))
							$user['userRole'] = _XALKY_MODERATOR;
				if (!isset($user['userRole']))
					$user['userRole'] = _XALKY_USER;
				return $user;
			}
		} else {
			// Guest users:
