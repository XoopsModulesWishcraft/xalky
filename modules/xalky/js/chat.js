...
var xalkyChat = {
	...
	setStartChatHandler: function() {
		if(this.dom['inputField']) {
			this.dom['inputField'].onfocus = function() {
				...
				xalkyChat.dom['inputField'].onfocus = '';
			}			
		}
	},
	...
	initEmoticons: function() {
		...
		for(var i=0; i<this.emoticonCodes.length; i++) {
			...
			this.DOMbuffer = this.DOMbuffer
						+ '<a href="javascript:xalkyChat.insertText(\''
	...
	initColorCodes: function() {
		if(this.dom['colorCodesContainer']) {
			...
			for(var i=0; i<this.colorCodes.length; i++) {
				this.DOMbuffer = this.DOMbuffer
					+ '<a href="javascript:xalkyChat.setFontColor(\''
					+ this.colorCodes[i]
	...
	loadFlashInterface: function() {
		if(this.dom['flashInterfaceContainer']) {
			this.updateDOM(
				...
				'<object id="xalkyChatFlashInterface" style="position:absolute; left:-100px;" '
				+'classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" '
				+'codebase="'
				+ window.location.protocol
				+'//download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" '
				+'height="1" width="1">'
				+'<param name="flashvars" value="bridgeName=xalkyChat"/>'
				+'<param name="src" value="'+this.dirs['flash']+'FABridge.swf"/>'
				+'<embed name="xalkyChatFlashInterface" type="application/x-shockwave-flash" pluginspage="'
				+ window.location.protocol
				+'//www.macromedia.com/go/getflashplayer" '
				+'src="'+this.dirs['flash']+'FABridge.swf" height="1" width="1" flashvars="bridgeName=xalkyChat"/>'
				+'</object>'
			);
			FABridge.addInitializationCallback('xalkyChat', this.flashInterfaceLoadCompleteHandler);
		}
	},
	
	flashInterfaceLoadCompleteHandler: function() {
		xalkyChat.initializeFlashInterface();
	},
	...
	socketConnect: function() {
		if(!this.socketIsConnected) {
			try {
				if(!this.socket && FABridge.xalkyChat) {
					this.socket = FABridge.xalkyChat.create('flash.net.XMLSocket');
	...
	socketConnectHandler: function(event) {
		xalkyChat.socketIsConnected = true;
		// setTimeout is needed to avoid calling the flash interface recursively:
		setTimeout('xalkyChat.socketRegister()', 0);
	},

	socketCloseHandler: function(event) {
		xalkyChat.socketIsConnected = false;
		if(xalkyChat.socket) {
			clearTimeout(xalkyChat.timer);
			xalkyChat.updateChat(null);
		}
	},
	
	socketDataHandler: function(event) {
		xalkyChat.socketUpdate(event.getData());
	},
	...
	socketIOErrorHandler: function(event) {
		...
		setTimeout('xalkyChat.addChatBotMessageToChatList(\'/error SocketIO\')', 0);
		setTimeout('xalkyChat.updateChatlistView()', 1);
	},

	socketSecurityErrorHandler: function(event) {
		// setTimeout is needed to avoid calling the flash interface recursively (e.g. sound on new messages):
		setTimeout('xalkyChat.addChatBotMessageToChatList(\'/error SocketSecurity\')', 0);
		setTimeout('xalkyChat.updateChatlistView()', 1);
	},
		...
		if(!isNaN(volume)) {
			...
			try {
				if(!this.soundTransform) {
					this.soundTransform = FABridge.xalkyChat.create('flash.media.SoundTransform');					
				}
	...
	soundLoadCompleteHandler: function(event) {
		...
		for(var key in xalkyChat.soundFiles) {
			// Get the sound key by matching the sound URL with the sound filename:
			if((new RegExp(xalkyChat.soundFiles[key])).test(sound.getUrl())) {
				// Add the loaded sound to the sounds list:
				xalkyChat.sounds[key] = sound;
			}
		}
	},
	...
	soundIOErrorHandler: function(event) {
		// setTimeout is needed to avoid calling the flash interface recursively (e.g. sound on new messages):
		setTimeout('xalkyChat.addChatBotMessageToChatList(\'/error SoundIO\')', 0);
		setTimeout('xalkyChat.updateChatlistView()', 1);
	},
	...
	makeRequest: function(url, method, data) {
		xalkyChat.setStatus('On');
		xalkyChat.retryTimer = setTimeout("xalkyChat.updateChat(null); xalkyChat.setStatus('Alert');", this.retryTimerDelay);
		try {
			var identifier;
			if(data) {
			...
			this.getHttpRequest(identifier).onreadystatechange = function() {
				try {
					xalkyChat.handleResponse(identifier);
				} catch(e) {
					try {
						clearTimeout(xalkyChat.timer);
					} catch(e) {
					...
					try {
						if(data) {
							xalkyChat.addChatBotMessageToChatList('/error ConnectionTimeout');
							xalkyChat.setStatus('Alert');
							xalkyChat.updateChatlistView();
						}
					} catch(e) {
						//alert(e);
					}
					try {				
						xalkyChat.timer = setTimeout('xalkyChat.updateChat(null);', xalkyChat.timerRate);
					} catch(e) {
						//alert(e);
					}
				}
			...
			this.timer = setTimeout('xalkyChat.updateChat(null);', this.timerRate);
		}
	},
	...
	handleResponse: function(identifier) {
		if (this.getHttpRequest(identifier).readyState == 4) {
			if (this.getHttpRequest(identifier).status == 200) {
				clearTimeout(xalkyChat.retryTimer);
				var xmlDoc = this.getHttpRequest(identifier).responseXML;
				xalkyChat.setStatus('Off');
			} else {
		...
		if(this.chatStarted) {
			...
			if(this.socketIsConnected) {
				...
				if(this.socketServerEnabled && !this.socketReconnectTimer) {
					...
					this.socketReconnectTimer = setTimeout('xalkyChat.socketConnect();', 60000);
				}
			}
			this.timer = setTimeout('xalkyChat.updateChat(null);', timeout);			
		}
	},
	...
	getUserNodeString: function(userID, userName, userRole) {
		if(this.userNodeString && userID == this.userID) {
			...
			var str	= '<div id="'
					...
					+ '"><a href="javascript:xalkyChat.toggleUserMenu(\''
	...
	getUserNodeStringItems: function(encodedUserName, userID, isInline) {
		...
		if(encodedUserName != this.encodedUserName) {
			menu 	= '<li><a href="javascript:xalkyChat.insertMessageWrapper(\'/msg '
					+ encodedUserName
					+ ' \');">'
					+ this.lang['userMenuSendPrivateMessage']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.insertMessageWrapper(\'/describe '
					+ encodedUserName
					+ ' \');">'
					+ this.lang['userMenuDescribe']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/query '
					+ encodedUserName
					+ '\');">'
					...
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/ignore '
					+ encodedUserName
					+ '\');">'
					+ this.lang['userMenuIgnore']
					+ '</a></li>';
			if (isInline) {
				menu	+= '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/invite '
					+ encodedUserName
					+ '\');">'
					+ this.lang['userMenuInvite']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/uninvite '
					+ encodedUserName
					+ '\');">'
					+ this.lang['userMenuUninvite']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/whereis '
					+ encodedUserName
					+ '\');">'
			...
			if(this.userRole == 2 || this.userRole == 3) {
				menu	+= '<li><a href="javascript:xalkyChat.insertMessageWrapper(\'/kick '
						+ encodedUserName
						+ ' \');">'
						+ this.lang['userMenuKick']
						+ '</a></li>'
						+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/whois '
						+ encodedUserName
						+ '\');">'
			...
			menu 	= '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/quit\');">'
					+ this.lang['userMenuLogout']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/who\');">'
					+ this.lang['userMenuWho']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/ignore\');">'
					+ this.lang['userMenuIgnoreList']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/list\');">'
					+ this.lang['userMenuList']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.insertMessageWrapper(\'/action \');">'
					+ this.lang['userMenuAction']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.insertMessageWrapper(\'/roll \');">'
					+ this.lang['userMenuRoll']
					+ '</a></li>'
					+ '<li><a href="javascript:xalkyChat.insertMessageWrapper(\'/nick \');">'
					+ this.lang['userMenuNick']
					+ '</a></li>';
			if(this.userRole == 1 || this.userRole == 2 || this.userRole == 3) {
				menu	+= '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/join\');">'
						+ this.lang['userMenuEnterPrivateRoom']
						+ '</a></li>';
				if(this.userRole == 2 || this.userRole == 3) {
					menu	+= '<li><a href="javascript:xalkyChat.sendMessageWrapper(\'/bans\');">'
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		return	'<div id="'
				...
				+ ' dir="'
				+ this.baseDirection
				+ '" onclick="xalkyChat.insertText(this.firstChild.nodeValue);">'
				+ userName
				+ '</span>'
				+ colon
	...
	getMessageDocumentID: function(messageID) {
		return ((messageID === null) ? 'xalkyChat_lm_'+(this.localID++) : 'xalkyChat_m_'+messageID);
	},
	...
	getUserMenuDocumentID: function(userID) {
		return 'xalkyChat_um_'+userID;
	},
	
	getInlineUserMenuDocumentID: function(menuID, index) {
		return 'xalkyChat_ium_'+menuID+'_'+index;
	},
	...
	getDeletionLink: function(messageID, userID, userRole, channelID) {
		if(messageID !== null && this.isAllowedToDeleteMessage(messageID, userID, userRole, channelID)) {
			...
			return	'<a class="delete" title="'
					...
					+ '" href="javascript:xalkyChat.deleteMessage('
					+ messageID
	...
	blinkOnNewMessage: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		if(this.settings['blink'] && this.lastID && !this.channelSwitch && userID != this.userID) {
			...
			this.blinkInterval = setInterval(
				'xalkyChat.blinkUpdate(\''+this.addSlashes(this.decodeSpecialChars(userName))+'\')',
	...
	replaceCommandInvite: function(textParts) {
		var inviteText = this.lang['invite']
							...
							.replace(
								/%s/,
								'<a href="javascript:xalkyChat.sendMessageWrapper(\'/join '
	...
	replaceCommandBans: function(textParts) {
		...
		for(var i=0; i<users.length; i++) {
			listUsers.push(
				'<a href="javascript:xalkyChat.sendMessageWrapper(\'/unban '
	...
	getInlineUserMenu: function(users) {
		...
		for(var i=0; i<users.length; i++) {
			...
			menu	+= '<a href="javascript:xalkyChat.toggleUserMenu(\''
	...
	replaceBBCodeCallback: function(str, p1, p2, p3) {
		...
		if(!xalkyChat.inArray(xalkyChat.bbCodeTags, p1)) {
			return str;
		}	
		// Avoid invalid XHTML (unclosed tags):
		if(xalkyChat.containsUnclosedTags(p3)) {
			return str;
		}			
		switch(p1) {
			case 'color':
				return xalkyChat.replaceBBCodeColor(p3, p2);
			case 'url':
				return xalkyChat.replaceBBCodeUrl(p3, p2);
			case 'img':
				return xalkyChat.replaceBBCodeImage(p3);
			case 'quote':
				return xalkyChat.replaceBBCodeQuote(p3, p2);
			case 'code':
				return xalkyChat.replaceBBCodeCode(p3);
			case 'u':
				return xalkyChat.replaceBBCodeUnderline(p3);
			default:
				return xalkyChat.replaceCustomBBCode(p1, p2, p3);
		}	
	},
	...
	replaceBBCodeColor: function(content, attribute) {
		if(this.settings['bbCodeColors']) {
			...
			if(!attribute || !this.inArray(xalkyChat.colorCodes, attribute))
				return content;								
	...
	replaceBBCodeImage: function(url) {
		if(this.settings['bbCodeImages']) {
			...
			return	'<a href="'
					...
					+'px;" src="'
					+url
					+'" alt="" onload="xalkyChat.updateChatlistView();"/></a>';
		}
		return url;
	},
	...
	replaceEmoticonsCallback: function(str, p1, p2, p3) {
		...
		if(p2) {
			var index = xalkyChat.arraySearch(p2, xalkyChat.emoticonCodes);							
			return 	xalkyChat.replaceEmoticons(p1)
				+	'<img src="'
				+	xalkyChat.dirs['emoticons']
				+	xalkyChat.emoticonFiles[index]
				+	'" alt="'
				+	p2
				+	'" />'
				+ 	xalkyChat.replaceEmoticons(p3);
		}
		return str;
	},
